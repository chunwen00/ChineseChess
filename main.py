...
def get_top_button_rects() -> tuple[pygame.Rect, pygame.Rect]:
    """取得頂部兩個按鈕的矩形區域（悔棋、重新開始），供繪製與點擊判斷共用。"""
    bar_h = 44  # 狀態列高度（需與 draw_status_bar 一致）
    btn_w = 130  # 按鈕寬度
    btn_h = 30  # 按鈕高度
    gap = 10  # 按鈕間距
    top = (bar_h - btn_h) // 2  # 垂直置中
    restart_rect = pygame.Rect(WINDOW_W - 16 - btn_w, top, btn_w, btn_h)  # 右側：重新開始
    undo_rect = pygame.Rect(restart_rect.left - gap - btn_w, top, btn_w, btn_h)  # 左側：悔棋
    return undo_rect, restart_rect
...
def draw_button(screen: pygame.Surface, rect: pygame.Rect, font: pygame.font.Font, label: str) -> None:
    ...
    screen.blit(text, text.get_rect(center=rect.center))  # 文字置中


def draw_status_bar(screen: pygame.Surface, status_font: pygame.font.Font, text: str) -> None:
    """在視窗頂部繪製醒目的狀態列 + 兩個操作按鈕（悔棋/重新開始）。"""
    bar_h = 44  # 狀態列高度
    ...
    # 右側按鈕
    undo_rect, restart_rect = get_top_button_rects()  # 取得按鈕區域
    draw_button(screen, undo_rect, status_font, "悔棋")  # 悔棋按鈕
    draw_button(screen, restart_rect, status_font, "重新開始")  # 重新開始按鈕

    # 狀態文字（放左邊，避免被按鈕蓋住）
    surf = status_font.render(text, True, fg)  # 渲染狀態文字
    screen.blit(surf, (16, 8))  # 貼到頂部左側
...
def draw_scene(
    ...
    draw_status_bar(screen, status_font, status_text)  # 狀態列（頂部）
...
def main() -> None:
    ...
    status_font = load_cjk_font(30)  # 狀態列字型（加大以便清楚看見）
    ...
    ai_config = AIConfig(depth=2, use_tt=True, time_limit_sec=1.5)
    ...
    def finalize_move(mover_side: str) -> None:
        ...
        if winner is not None:
            ...
            draw_scene(
                screen, board_font, piece_font, status_font, status_text,
    ...
    while running:  # 主迴圈：持續處理事件並重畫畫面
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                undo_rect, restart_rect = get_top_button_rects()
